...
function getModelChain(provider, env) {
  if (provider === 'gemini') {
    // gemini-2.5-flash-image is scheduled for shutdown Oct 2, 2026 — keeping it
    // first for now since it's still live and well-tested, with Google's forward
    // replacement as an automatic fallback so this keeps working after that date
    // without needing another manual fix.
    return putFirst(['gemini-2.5-flash-image', 'gemini-3.1-flash-image-preview'], env.GEMINI_MODEL_IMAGE);
  }
